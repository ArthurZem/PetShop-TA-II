...
describe("ProdutoController", ()=>{
    const mockProdutoService: any = {
        findById: jest.fn().mockRejectedValue(Error("Erro ao tentar obter Produto.")),
    };

    const produtoController = new ProdutoController(mockProdutoService)

    it("deve buscar um Produto por Id e falhar com erro 500",async () => {
        ...
        expect(mockResult.json).toHaveBeenCalledWith({ error: "Erro ao tentar obter Produto." });
